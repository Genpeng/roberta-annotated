...
class InputExample(object):
    """A single training/validation/test example for token classification."""

    def __init__(self, guid, text, label=None):
        """Constructs a `InputExample` object.

        Args:
            guid: Union[int, str], unique id for the example.
            text: str, the input untokenized text
            label: Option[str], the label of the example
        """
        self.guid = guid
        self.text = text
        self.label = label
...
class PaddingInputExample(object):
    """Fake example, so the number of input examples is a multiple of the batch size.
...
class DataProcessor(object):
    """Base class for data converters for token classification datasets."""
...
def main(_):
    ...
    model_fn = model_fn_builder(
        ...
        num_labels=len(label_list) + 1,  # ? why len(label_list) + 1
    ...
    # create a `TPUEstimator` instance which needs `run_config` and `model_fn`
    estimator = tf.contrib.tpu.TPUEstimator(
        model_fn=model_fn,  # `model_fn` closure
        config=run_config,  # `RunConfig` instance
        use_tpu=FLAGS.use_tpu,
